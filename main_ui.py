...
class TrackerGui(MainUi):
    ...
    def update_cam_list(self):
        cam_list = [cam.__str__() for cam in self.cameras.values()]
    ...
    def update_res_list(self):
        ...
        if self.eye_cam:
            self.eye_res_combo_box.addItems(self.eye_cam.str_resolutions)
        if self.seeing_cam:
            self.seeing_res_combo_box.addItems(self.seeing_cam.str_resolutions)
    ...
    def setup_ui(self, MainWindow):
        ...
        self.thread = QThread()
        self.f_updater.frame_ready.connect(self.on_frame_ready)
        self.f_updater.moveToThread(self.thread)
        self.thread.started.connect(self.f_updater.process)
        self.thread.start()
        
        self.f_updater.eye_cam = self.eye_cam
        self.f_updater.seeing_cam = self.seeing_cam
        ...
        self.eye_cam_btn.pressed.connect(self.f_updater.show_eye)
        self.seeing_cam_btn.pressed.connect(self.f_updater.show_seeing)
